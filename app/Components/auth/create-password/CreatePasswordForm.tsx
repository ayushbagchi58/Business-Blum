...
import { CreatePasswordFormData, PasswordStrength } from "./types";
import { createPasswordData } from "./data";
...
export default function CreatePasswordForm() {
  ...
  const passwordStrength: PasswordStrength = useMemo(() => {
    ...
    if (!password) {
      return {
        ...
        label: "",
        color: "",
        requirements: createPasswordData.requirements.map((req) => ({
          id: req.id,
          label: req.label,
          met: false,
        })),
    ...
    const requirements = createPasswordData.requirements.map((req) => ({
      id: req.id,
      label: req.label,
    ...
    const strengthLevel =
      createPasswordData.strengthLevels[metCount] ||
      createPasswordData.strengthLevels[0];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      toast.error(
        "Invalid password creation link. Please use the link from your email."
      );
      return;
    }

    try {
      console.log("🔐 Creating password with token:", token);
      ...
      >
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-[#08122E]">
            {createPasswordData.title}
          </h1>
          <p className="mt-2 text-sm text-gray-600">
            {createPasswordData.subtitle}
          </p>
        </div>
